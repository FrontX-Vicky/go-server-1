	...
	if len(itemCols) > 0 {
		for index, item := range lineItems {
			itemFields := map[string]any{
				...
				"invoice_master_id":       salesInvoiceID,
				"line_order":              index + 1,
				"description":             item.Description,
				"hsn_sac":                 item.HSN,
				"quantity":                item.Quantity,
				"unit":                    item.Unit,
				"item_currency_code":      firstNonEmpty(owner.Currency, "INR"),
				"item_currency_rate":      1.0,
				"foreign_unit_price":      item.UnitPrice,
